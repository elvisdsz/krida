...
class MetricSeries {
  ...
  private collectSamples(): number[] {
    ...
    return [...this._samples.slice(this._nextIndex), ...this._samples.slice(0, this._nextIndex)];
  }
}
...
export class PerformanceMonitor {
  ...
  constructor(options: PerformanceMonitorOptions = {}) {
    ...
    if (!Number.isInteger(windowSize) || windowSize <= 0) {
      throw new Error(
        `PerformanceMonitor windowSize must be a positive integer, got ${windowSize}`,
      );
    }
  ...
  snapshot(): PerformanceSnapshot {
    ...
    return {
      ...
      handPostProcessing:
        this._handPostProcessing.count > 0 ? this._handPostProcessing.stats() : null,
      posePostProcessing:
        this._posePostProcessing.count > 0 ? this._posePostProcessing.stats() : null,
    };
  }
