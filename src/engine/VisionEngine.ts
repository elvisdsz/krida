...
export class VisionEngine {
  ...
  static async create(
    ...
    if (!options.handLandmarkerEnabled && !options.poseLandmarkerEnabled) {
      throw new Error(
        "VisionEngine: At least one of handLandmarkerEnabled or poseLandmarkerEnabled must be true.",
      );
    }

    const engine = new VisionEngine();
    engine._smoothingAlpha = options.smoothingAlpha ?? VisionEngineDefaults.smoothingAlpha;
    const visionTaskFilesetPath =
  ...
  private loadHandLandmarker = async (
    ...
    let baseOptions: { delegate?: "CPU" | "GPU" } & (
      { modelAssetPath: string } | { modelAssetBuffer: Uint8Array }
    );
    let downloadMs = 0;
    if (performanceMonitor == null) {
      baseOptions = {
        modelAssetPath: url,
        delegate: options.delegate ?? VisionEngineDefaults.delegate,
      };
    } else {
      const fetched = await measureAsync(() => fetchModelBuffer(url));
      baseOptions = {
        modelAssetBuffer: fetched.value,
        delegate: options.delegate ?? VisionEngineDefaults.delegate,
      };
    ...
    const { value: handLandmarker, ms: loadMs } = await measureAsync(() =>
      HandLandmarker.createFromOptions(visionTaskFileset, {
        baseOptions,
  ...
  private loadPoseLandmarker = async (
    ...
    const { value: poseLandmarker, ms: loadMs } = await measureAsync(() =>
      PoseLandmarker.createFromOptions(visionTaskFileset, {
        baseOptions,
        runningMode: "VIDEO",
      }),
    );
  ...
  getTrackerResult = (
    ...
    if (this._isDestroyed) {
      throw new Error(
        "VisionEngine: getTrackerResult called after destroy(). Create a new VisionEngine instance.",
      );
    }
  ...
  private getHandResult = (
    ...
    const normalizedStartTimeMs = VisionEngine.getMonotonicStartTimeMs(
      startTimeMs,
      this._lastHandStartTimeMs,
    );
  ...
  private filterLandmarks(
    ...
    landmarkFilters: LandmarkFilter[],
  ): NormalizedLandmark[][] {
...
async function fetchModelBuffer(url: string): Promise<Uint8Array> {
  ...
  if (!res.ok) {
    throw new Error(
      `VisionEngine: failed to fetch model from ${res.url || url}: ${res.status} ${res.statusText}`,
    );
  }
