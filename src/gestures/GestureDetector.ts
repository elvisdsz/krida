...
export abstract class GestureDetector<TResult extends { startTimeMs: number }> {
    ...
    constructor(defaultName: string, defaults: Required<Omit<GestureOptions, "name">>, options: GestureOptions = {}) {
        ...
        if (this._deactivateAt > this._activateAt) {
            throw new Error(`Gesture '${this.name}': deactivateAt (${this._deactivateAt}) must be less than or equal to activateAt (${this._activateAt})`);
        }
    }
